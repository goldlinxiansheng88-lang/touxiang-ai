...
CONFIG_ENTRIES: tuple[ConfigEntry, ...] = (
    ...
    ConfigEntry(
        ...
        label="定价页 · 订阅链接",
        description="完整 HTTPS URL（如 Stripe Customer Portal、Stripe Payment Link、Creem 订阅页）。保存后前台 /pricing 「订阅」按钮跳转此地址。留空则隐藏该按钮。",
        group="支付与会员",
        is_secret=False,
        required=False,
    ),
    ConfigEntry(
        ...
        label="定价页 · 积分包 / 充值链接",
        description="完整 HTTPS URL（如 Stripe Payment Link、Lemon 商品页）。保存后前台 /pricing 「积分包」按钮跳转。留空则隐藏。",
        group="支付与会员",
        is_secret=False,
        required=False,
    ),
